...
# plotting function
def plot_ppi_vr(ldict):
    """
    Plot lidar PPI of radial velocity.

    :param dictionary dict: Plotting dictionary. Must contain
        - x
        - y
        - vr
        - vort
    ...
    fig, ax = plt.subplots(figsize=(4,5.15),
                            ...
                            dpi=200)
    # fig title
    fig.suptitle(f"{ldict['tsdt'].time()} UTC, El={ldict['elplt']:.2f}$\\bf{{^{{\\circ}}}}$",
                 fontsize=10, 
                 fontweight='bold')

    # radial velocity
    ax.set_title("Radial Velocity")
    pcm = ax.pcolormesh(ldict['x'].T/1e3, 
                        ldict['y'].T/1e3, 
                        ldict['vr'],
                        cmap='Carbone42',
                        vmin=-38, 
                        vmax=38)
    plt.colorbar(pcm, ax=ax, orientation="horizontal", label="$V_r$ [m s$^{-1}$]", pad=0.03)
    ...
    if 'xlims' in ldict.keys():
        ax.set_xlim(ldict['xlims'])
        ax.set_ylim(ldict['ylims'])
